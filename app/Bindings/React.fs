...
let inline ofImport
    ...
    : ReactElement
    =
    React.createElement (import importMember importPath, mkPropObject props, children)

let inline ofImportWithoutProps
    (importMember : string)
    (importPath : string)
    (children : ReactElement seq)
    : ReactElement
    =
    React.createElement (import importMember importPath, null, children)

let inline ofImportWithoutChildren (importMember : string) (importPath : string) (props : seq<#IProp>) : ReactElement =
    React.createElement (import importMember importPath, mkPropObject props, Array.empty)

/// Instantiate a DOM React element
let inline domEl (tag : string) (props : IHTMLProp seq) (children : ReactElement seq) : ReactElement =
    React.createElement (tag, mkPropObject props, children)
...
let inline voidEl (tag : string) (props : IHTMLProp seq) : ReactElement =
    React.createElement (tag, mkPropObject props, [])
...
let inline svgEl (tag : string) (props : IHTMLProp seq) (children : ReactElement seq) : ReactElement =
    React.createElement (tag, mkPropObject props, children)
...
type Browser.Types.Event with
    ...
    [<Emit("$0.target.value")>]
    member this.Value : string = (this.target :?> HTMLInputElement).value

    /// Access the checked property from target
    /// Equivalent to `(this.target :?> HTMLInputElement).checked`
    [<Emit("$0.target.checked")>]
    member this.Checked : bool = (this.target :?> HTMLInputElement).``checked``
...
module rec Props =
    ...
    let inline Data (key : string, value : obj) : HTMLAttr = !!("data-" + key, value)
