    ...
    List.fold (fun acc ev ->
        ...
        | LocationAdded la ->
            ...
            if Map.containsKey creator acc then
                ...
                |> (+) 1
                |> fun v -> Map.add creator v acc
            else
                Map.add creator 1 acc
    ) Map.empty events
...
let private useUserScore () =
    ...
    let scores = React.useMemo ((fun () -> getScores eventCtx.Events), [| eventCtx.Events |])
            ...
            if auth0.isAuthenticated then
                ...
                |> Promise.map (fun json ->
                    ...
                    | Ok users ->
                        users
                        |> List.choose (fun (k, u) ->
                            let userScore = Map.tryFind k scores |> Option.defaultValue 0
                            if userScore > 0 then
...
let private LeaderboardPage =
    React.functionComponent ("LeaderboardPage", (fun () ->
        ...
        let userRows =
            ...
            |> List.map (fun us ->
                tr [ Key us.Name ] [
                    td [] [ img [ Src us.Picture; HTMLAttr.Height "30px"; HTMLAttr.Width "30x" ] ]
                    td [] [ str us.Name ]
        ...
        page [] [
            h1 [] [ str "Klassement" ]
            table [ classNames [ Bootstrap.Table; Bootstrap.TableStriped; Bootstrap.TableLight ] ] [
                thead [] [
                    tr [] [
                        th [] []
                        th [] [ str "Naam" ]
